...
//Version Info
// 1.0 Initial version
// 1.1 Add support for foams
// 1.2 fix for spurious exception entries
// 1.3 fix issue with identification of non foams records
// 1.4 Change Other consumer selection rules
// 1.5 Add Industrial Consumer
// 1.6 Change to 120 weeks import
// 1.7 Remove VICSKUTMP output
...
var
    ...
    RecCount,WCount,CCount,ECount,CompanyID,IndexNo,RecNo:Integer;
    OutStr: String;
    InFile,OutFile,ExceptFile,DisagLkupFile:TextFile;
    ...
    FoundCat:Boolean;
    TheData,DesignCode:String;
    Loc1,Loc2,Loc3,Loc4,SupplyChain,Country:String;
    GroupCode,PlnGroup,WHcode,Brand,Owner1{, SourceLocation}, Locn:String;
begin
  ...
  try
    ...
    {SourceLocation := GetParam('location Code'); }
    ...
    Rewrite(ExceptFile);
    WriteLn(ExceptFile,'Record Number,Error Message,Product,Division');
    ...
    begin
      ...
      if (DataRecord[1] <> '') then //and (ReCCount > 1) then
      begin

        Locn := DataRecord[_Location];
        ...
        if Trim(CustCat) = '' then CustCat := 'ZZ';

        if CompanyID in [_Sleepmaker] then
        begin

          IndexNo := ProductList.IndexOf(Locn + '|' + ProdCode);

          if IndexNo >= 0 then
          begin
            Style := (ProductList.Objects[IndexNo] as TProductPB).Data[1];
            GetWarehouseLocation_SL(Trim(Locn),Loc1,Loc2,Loc3,Country);
            CustCat := Trim(DataRecord[_CustomerCat]);
            //GetTerritoryLocationCode(Trim(DataRecord[_Territory]),Loc1);
            if AnsiPos('_APP',UpperCase(Trim(DataRecord[_Location]))) >= 1 then  //Sleepmaker logic
            begin
              //only those with model code
              if (Style <> '' )   then
              ...
              begin
               ...
               // if Loc1 = 'VICSKU' then
                 // Loc3 := 'VICSKUTMP';
                if (UpperCase(Trim(Locn)) <> 'ADEL_APP') then
                  Loc4 := Copy(Loc1,1,3) + 'STL' + TempLoc3
                else
                begin
                  Loc4 := 'SAUSTL' + TempLoc3 ;
                end;
              end;
            end
            else if AnsiPos('_JBA',UpperCase(Trim(DataRecord[_Location]))) >= 1 then  //Foams logic
            begin
              ...
              if Loc4 <> '' then
              begin
                  //Populate Loc3 with the appropriate Foams Intermediate Disag Location
                  Loc3 := GetFoamsDisagLoc(Loc4, Loc1)
              end;
            end;
          end
          else
        ...
        begin
          ...
          if Loc1 <> '' then
            WriteLn(OutFile,Loc1 + OutStr+','+ProdCode);

          if Loc3 <> '' then
            WriteLn(OutFile,Loc3 + OutStr+','+ProdCode);

          if (Loc4 <> '') and (Style<>'') and (AnsiPos('_APP',UpperCase(Trim(DataRecord[_Location]))) >= 1) then
            WriteLn(OutFile,Loc4 + OutStr+','+Style);

          if (Loc4 <> '')  AND (AnsiPos('_JBA',UpperCase(Trim(DataRecord[_Location]))) >= 1) then
          begin
            WriteLn(OutFile,Loc4 + OutStr+','+ProdCode);
            //Add sales to the intermediate disag locations as well since this loc4 is a customer location
