...
begin
  ...
  NewParam(ParamNo,'Create Lookup','',True,esPickList,ptYesNo);
  NewParam(ParamNo,'Generate Lookup From DB','',False,esPickList,ptYesNo);      //Determines if the lookup is created from the database and then tweaked from the master extract.  Used when partial master import is implemented.
  NewParam(ParamNo,'Process New Item Files','',True,esPickList,ptYesNo);
  NewParam(ParamNo,'Cost File Name','',False,esEllipsis,ptFile);
  NewParam(ParamNo,'Customer Forecast Mode','',True,esPickList,ptYesNo);
...
procedure TfrmParameters.LoadSource;
var
  FNAme:String;

begin

  if GetCompanyID(vleParameters.Values['Company']) = _Tontine then
  begin
    FName := ExtractFilePath(ParamStr(0));
    FName := FName + 'master_source.dat';
    vleSource.Strings.LoadFromFile(FName);
  end;

end;

procedure TfrmParameters.SaveSource;
var
  FNAme:String;
...
begin
  ...
  begin
    FName := ExtractFilePath(ParamStr(0));
    FName := FName + 'master_source.dat';
    vleSource.Strings.SaveToFile(FName);
  end;
