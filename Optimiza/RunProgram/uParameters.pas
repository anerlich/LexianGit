...
type
  ...
  TParamType = (ptYesNo, ptFile, ptiniFile, ptString, ptInfoOnly, ptDays);
...
uses uSelectOneLocation, uSelectDayListOpt;
...
begin
  ...
  ParamNo := 0;
  NewParam(ParamNo,'Program to run','',True,esEllipsis,ptFile);
  NewParam(ParamNo,'Parameters','',False,esSimple,ptString);
  NewParam(ParamNo,'Program ini file','',True,esEllipsis,ptiniFile);
  NewParam(ParamNo,'Run On Days','',True,esEllipsis,ptDays);
...
begin
  ...
  if  VleParameters.Keys[RowNo] = 'Program ini file' then
  begin
    if opendialog2.Execute then
      VleParameters.Values['Program ini file'] := opendialog2.FileName;
  end;

  if  VleParameters.Keys[RowNo] = 'Run On Days' then
  begin
    frmSelectDayList.SelectedDays := VleParameters.Values['Run On Days'];
    frmSelectDayList.ShowModal;

    if frmSelectDayList.SelectedDays <> '' then
      VleParameters.Values['Run On Days'] := frmSelectDayList.SelectedDays;
  end;
