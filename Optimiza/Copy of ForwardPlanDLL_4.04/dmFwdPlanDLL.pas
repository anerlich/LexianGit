...
(*    EffShutdownStart           : TDateTime;    //       D -=-     -=- The stock build shutdown period start date.
   ...
   {$endif}
     ...
     //###Lexian Change - moved from private to public
...
begin
  ...
  // Some rles apply if the following fields are zero
  if FParams.RC <= 0 then
    FParams.RC := 0.04;
  if FParams.RP <= 0 then
    FParams.RP := 0.04;
  if FParams.LT <= 0 then
    FParams.LT := 0.04;   
