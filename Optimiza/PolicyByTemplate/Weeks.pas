...
uses
  ...
  Dialogs, Grids, ValEdit, StdCtrls, Buttons,StrUtils, ExtCtrls,IdGlobal,
  Menus, ExtEdit;
...
type
  ...
  private
    ...
    procedure SaveFiles;
    procedure ConvFiles(rCount:integer);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        TempName := ParamStr(0);
        FParamFile := AnsiReplaceStr(UpperCase(TempName),'WEEKSCONVERT.EXE','PolicyByTemplate.dat');
      end;
...
begin
  ...
  btnConvert.Enabled := Panel3.Enabled;
...
var
  ...
  SayStr,ErrMsg,ExceptName:String;
  ExceptFile:TextFile;
begin
 ...
 try
  ...
  begin
    ...
    frmStatus.Say('');
    frmStatus.Say('===========================================================================================');
    frmStatus.Say('ID :' + VleUpdates.cells[0,rCount]);
 ...
 frmStatus.Say('Finshed at '+DateTimeToStr(Now));
  ...
  if ErrCount=0 then
  begin
    if FileExists(ExceptName) = true then
    begin
...
procedure TfrmMain.ConvFiles(rCount:integer);
var
lCount , flag: integer;
begin
  for lCount := 1 to grdLocs.RowCount-1 do
  begin
    ...
    if Trim(grdLocs.Cells[0,lCount]) <> '' then
    begin
      if (chkLT.Checked=true) and (Trim(grdLocs.Cells[1,lCount])<>'') then
        grdLocs.Cells[1,lCount] := FloatToStr(strtofloat(grdLocs.Cells[1,lCount]) * 4);

      if (chkRC.Checked=true) and (Trim(grdLocs.Cells[2,lCount])<>'') then
        grdLocs.Cells[2,lCount] := FloatToStr(strtofloat(grdLocs.Cells[2,lCount]) * 4);

      if (chkRP.Checked=true) and (Trim(grdLocs.Cells[3,lCount])<>'') then
        grdLocs.Cells[3,lCount] := FloatToStr(strtofloat(grdLocs.Cells[3,lCount]) * 4) ;

      if (chkSS.Checked=true) and (Trim(grdLocs.Cells[5,lCount])<>'') then
        grdLocs.Cells[5,lCount] := FloatToStr(strtofloat(grdLocs.Cells[5,lCount]) * 4);

      if vleSettings.Values[grdLocs.Cells[0,lCount]]<> grdLocs.Rows[lCount].DelimitedText then   Flag:=1;


      if vleSettings.FindRow(grdLocs.Cells[0,lCount],RCount) then
        vleSettings.Values[grdLocs.Cells[0,lCount]]:= AnsiReplaceStr(grdLocs.Rows[lCount].DelimitedText,'" "','')
      else
        vleSettings.InsertRow(grdLocs.Cells[0,lCount],grdLocs.Rows[lCount].DelimitedText,True);
      Flag := 0;
    end;
