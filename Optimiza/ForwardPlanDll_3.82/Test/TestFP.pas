...
function GetRecommendedOrders(EnginePointer : Pointer;
                              ...
                              LocalOutputForecast : POutputForecast;
                              BOMOutputForecast : POutputForecast;
                              DRPOutputForecast : POutputForecast;
                              COOutputForecast : POutputForecast;
                              ...
                              Excess : PExcess;
                              MaxStock : PMaxStock) : integer; stdcall;
                              external 'OptimizaFP.DLL';
function InitialiseEngine(var EnginePointer : Pointer;
                          LocationParams : PLocationParams;
                          Calendar : PCalendar) : integer; stdcall; external 'OptimizaFP.DLL';
function FinaliseEngine(var EnginePointer : Pointer) : integer; stdcall; external 'OptimizaFP.DLL';
function DLLVersion : PChar; stdcall; external 'OptimizaFP.DLL';
...
type
  ...
  private
    ...
    FLocalOutputForecast : TOutputForecast;
    FBOMOutputForecast : TOutputForecast;
    FDRPOutputForecast : TOutputForecast;
    FCOOutputForecast : TOutputForecast;
...
begin
  if dlgSave.Execute then begin
    ...
    try
      ...
      if FLocationParams.ErrorCode = 0 then begin
  ...
  {ivb}           FExcess.EX[n] := 0;
                  FLocalOutputForecast.OutputForecast[n] := 0;
                  FBOMOutputForecast.OutputForecast[n] := 0;
                  FDRPOutputForecast.OutputForecast[n] := 0;
                  FCOOutputForecast.OutputForecast[n] := 0;
                  FMaxStock.Max[n] := 0;
                end;  {for}
  (*              if GetItemData.FieldByName('ProductCode').AsString = '0810150000' then begin
                ...
                ErrorCode := GetRecommendedOrders(Engine, @FLocationParams, @FItemParams,
                                        @FPurchaseOrder, @FCustomerOrder,
                                        @FLocalForecast, @FBOMForecast, @FDRPForecast,
                                        @FLocalOutputForecast, @FBOMOutputForecast, @FDRPOutputForecast,
                                        @FCOOutputForecast, @FRecommendedOrders, @FOpeningStock,
                                        @FCalculatedReceipts, @FBackOrders, @FCumLostSales, @FExcess, @FMaxStock);
  ...
  {ivb}                  ResultString := ResultString + ',' + Format('%10.0f',[FExcess.EX[n]]);
    ...
    *)
        ...
        ShowMessage(FLocationParams.ErrorMessage);
      end;
    finally
