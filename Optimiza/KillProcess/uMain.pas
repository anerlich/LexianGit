...
const
  ...
  _Opt_Optimiza=1;
  _Opt_StopFirebird=2;
  _Opt_fb_inet_server=3;
  _Opt_StartFirebird=4;
  _Opt_MismatchReport=5;
  _Opt_LateOrderReport=6;
...
type
  ...
  private
    ...
    procedure PerformTask(TaskID:Integer);
    procedure FirebirdAction(anAction:Integer);
...
begin
  ...
  if chkTasks.Checked[_Opt_Scheduler] then
    KillOptimizaProcess('Scheduler.exe');

  if chkTasks.Checked[_Opt_Optimiza] then
    KillOptimizaProcess('Optimiza.exe');

  if chkTasks.Checked[_Opt_MismatchReport] then
    KillOptimizaProcess('MismatchReportPB.exe');

  if chkTasks.Checked[_Opt_LateOrderReport] then
    KillOptimizaProcess('LateCustomerOrderReportPB.exe');

  if chkTasks.Checked[_Opt_StopFirebird] then
    FireBirdAction(_Opt_StopFirebird);

  if chkTasks.Checked[_Opt_fb_inet_server] then
    KillAll_ib_net;

  if chkTasks.Checked[_Opt_StartFirebird] then
    FireBirdAction(_Opt_StartFirebird);
...
procedure TForm1.PerformTask(TaskID: Integer);
begin
  Memo1.Clear;
  Memo2.Clear;
  ListTheProcesses;
  ...
  Case TaskID of
    _Opt_Scheduler: KillOptimizaProcess('Scheduler.exe');
    _Opt_Optimiza: KillOptimizaProcess('Optimiza.exe');
  end;
...
begin
  PerformTask(_opt_Optimiza);
  SayMessage('### Done with Optimiza');
  ListTheProcesses;
end;

procedure TForm1.FirebirdAction(anAction: Integer);
var
  CmdLine,CurrentDirectory:String;
begin

  CurrentDirectory := ExtractFilePath(ParamStr(0));

  if anAction = _Opt_StopFirebird then
    CmdLine := 'net Stop "Firebird Guardian - DefaultInstance"';

  if anAction = _Opt_StartFirebird then
    CmdLine := 'net Start "Firebird Guardian - DefaultInstance"';

  SayMessage('Current Directory ' + CurrentDirectory);
  SayMessage('Executing ' + CmdLine);
  Sleep(3000);

  ExecuteFileWait(CmdLine,CurrentDirectory)  ;
  Sleep(3000);
