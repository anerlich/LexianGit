...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls,Clipbrd, Buttons, Grids, ValEdit, FileCtrl;
...
begin
  ...
  ParamNo := 0;
  NewParam(ParamNo,'Input Folder','',False,esEllipsis,ptFile);
  NewParam(ParamNo,'Exception Folder','',False,esEllipsis,ptFile);
  NewParam(ParamNo,'Header Lines','',False,esSimple,ptString);
  NewParam(ParamNo,'Columns','',False,esSimple,ptString);
  NewParam(ParamNo,'Table Name','',False,esSimple,ptString);
  NewParam(ParamNo,'Map Columns to database name','',true,esSimple,ptInfoOnly);
...
begin
  ...
  begin
    //if selectdirectory('Select a folder', getCurrentDir(), folder) then
    folder :=  getCurrentDir();
    if selectdirectory(folder, options, 0) then
    begin
      VleParameters.Values[ParamName] := folder;
    end;
