...
uses
  ...
  Db, IBCustomDataSet, IBSQL, IBDatabase, Registry, FwdPlan;
...
begin
  ...
  for Cnt := 0 to FFP.Cnt-1 do begin //Loop thru all days
    ...
    if WeekNo <= MAX_WEEKS_ARRAY_SIZE then begin
      ...
      //Use maximum value
      if FFP.Dat[Cnt].BO > WeekBO[WeekNo] then
...
begin
    ...
    for Cnt := 0 to FFP.Cnt-1 do begin //Loop thru all days
      ...
      if FFP.Dat[Cnt].BO > MonthBO[CalNo] then
...
begin
  ...
  FFC.Cnt := 0;
  if (sqlFC.RecordCount > 0) then begin
    CarryVal := 0;
  //  DifVal := 0;
    AvgForecast := 0;
    for n := 0 to FOriginalHorizon -1 do begin
      IF N >= (FOriginalHorizon ) - FAvrFCPeriod then
        AvgForecast := AvgForecast +sqlFC.FieldByName('FORECAST_' + IntToStr(n)).AsDouble;
      ...
      if (IntVal <> DoubleVal) then begin
        DifVal := DoubleVal - IntVal;
        if (DifVal >= 0.5) then begin
          IntVal := IntVal + 1;
          CarryVal := (-1 + DifVal)
        end
        else begin
  ...
  If FNoperiodsFC <> FOriginalHorizon then
    begin
      If FAvrFCPeriod > 0 then
        AvgForecast := Avgforecast / FAvrFCPeriod
      else
        AvgForecast := 0;
     for n := FOriginalHorizon to FNoPeriodsFC -1 do begin
