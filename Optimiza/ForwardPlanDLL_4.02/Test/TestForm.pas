...
begin
  with FwdPlanDLLDataModule do begin
    ...
    edtSSDays.Text := IntToStr(Trunc(ELRound(dmTest.qryItemSAFETYSTOCK.AsFloat * FNoDaysInPeriod)));
    edtRCDays.Text := IntToStr(Trunc(ELRound(dmTest.qryItemREPLENISHMENTCYCLE.AsFloat * FNoDaysInPeriod)));
    edtRPDays.Text := IntToStr(Trunc(ELRound(dmTest.qryItemREVIEWPERIOD.AsFloat * FNoDaysInPeriod)));
    edtLTDays.Text := IntToStr(Trunc(ELRound(dmTest.qryItemLEADTIME.AsFloat * FNoDaysInPeriod)));
...
begin
  with FwdPlanDLLDataModule do begin
    Intrans := dmTest.DefaultTrans.InTransaction;
    if not InTRans then
     dmTest.DefaultTrans.StartTransaction;
...
begin
  ...
  edtProductCode.Text := '03112J_01 _05+';    //Example A Item at HJ- should have orders
  btnLoadItem.Click;
end;
